...
def _split_path_and_filename(path: pathlib.Path):
  directory, filename = os.path.split(path.as_posix())
...
class VolumeExtractor:
  ...
  def __init__(self, input_file_path: str, output_file_path: str, version: str):
    ...
    self.path, self.file_name = _get_path_info(
        pathlib.Path(output_file_path)
    )
  ...
  def process(self):
    ...
    # TODO: Fix this code that is commented
    # if len(v_entities) == 0:
  ...
  def create_model(self, obj: Obj):
    ...
    output_file_path = self.path + "/" + obj.name + ".msh"
